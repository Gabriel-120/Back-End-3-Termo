...
<body class="pokedex-bg min-h-screen flex flex-col items-center justify-center px-4 py-8">
    ...
    <nav class="mb-4 flex w-full max-w-[1200px] flex-wrap items-center justify-between gap-3">
        <div class="flex flex-wrap gap-2">
            <a href="{{ route('pokemon.list') }}"
                class="rounded-full bg-white px-4 py-2 text-xs font-bold uppercase tracking-[0.2em] text-[#0f172a] shadow transition hover:bg-[#fef3c7]">Lista</a>
            <a href="{{ route('pokemon.create.choice') }}"
                class="rounded-full bg-[#ef4444] px-4 py-2 text-xs font-bold uppercase tracking-[0.2em] text-white shadow transition hover:bg-[#dc2626]">Criar/Fundir</a>
        </div>

        @if(session('success'))
            <div class="rounded-full bg-[#22c55e] px-4 py-2 text-xs font-bold uppercase tracking-[0.18em] text-[#052e16] shadow">
                {{ session('success') }}
            </div>
        @endif
    </nav>
                            ...
                            <div class="mt-3 grid gap-3">
                                @if(count($evolutions))
                                    @foreach($evolutions as $evo)
                                        @php
                                            $evoId = $evo['id'] ?? null;
                                            $isCurrentEvolution = (string) $evoId === (string) $pokemon['id'];
                                            $evoClass = $isCurrentEvolution ? 'border-[#f97316] bg-[#fee2b3]' : 'border-transparent bg-white/90 hover:bg-white';
                                        @endphp

                                        @if($evoId)
                                            <button type="button" onclick="selectEvolution({{ $evoId }})"
                                                class="flex items-center gap-3 rounded-3xl border px-3 py-3 transition text-left w-full {{ $evoClass }}">
                                        @else
                                            <div class="flex items-center gap-3 rounded-3xl border px-3 py-3 text-left w-full border-transparent bg-white/70">
                                        @endif
                                                <img src="{{ $evo['image'] ?? 'https://via.placeholder.com/80?text=?' }}"
                                                    alt="{{ $evo['name'] }}" class="h-14 w-14 rounded-full object-contain bg-white p-1" />
                                                <div>
                                                    <p class="text-[10px] uppercase tracking-[0.28em] text-[#475569]">#{{ $evoId ? str_pad($evoId, 3, '0', STR_PAD_LEFT) : '---' }}</p>
                                ...
                                <div class="mt-3 grid gap-3">
                                    @foreach($variants as $variant)
                                        @php
                                            ...
                                            $variantClass = ! empty($variant['is_current']) ? 'border-[#8b5cf6] bg-[#ede9fe]' : 'border-transparent bg-white/90 hover:bg-white';
                                        @endphp

                                        @if($variantId)
                                            <button type="button" onclick="selectVariant({{ $variantId }})"
                                                class="flex items-center gap-3 rounded-3xl border px-3 py-3 transition text-left w-full {{ $variantClass }}">
                                        @else
                                            <div class="flex items-center gap-3 rounded-3xl border px-3 py-3 text-left w-full border-transparent bg-white/70">
                                        @endif
                                                ...
                                                <div>
                                                    <p class="text-[10px] uppercase tracking-[0.28em] text-[#475569]">#{{ $variantId ? str_pad($variantId, 3, '0', STR_PAD_LEFT) : '---' }}</p>
                                                    <p class="text-sm font-semibold text-[#0f172a]">{{ $variant['form'] }}</p>
                                                    @if(! empty($variant['is_current']))
            ...
            <div class="lg:flex-1 bg-[#111827] p-6 lg:p-8 text-white">
                ...
                <div class="grid gap-6">
                    ...
                    <div class="flex flex-wrap gap-4">
                        <button onclick="window.location.href = '{{ route('pokemon.game') }}';"
                            class="rounded-full bg-[#ef4444] px-6 py-3 text-sm font-semibold uppercase tracking-[0.24em] text-white transition hover:bg-[#dc2626]">Central de Jogos</button>
                        <button onclick="speakPokemonInfo()"
                            class="rounded-full bg-[#8b5cf6] px-6 py-3 text-sm font-semibold uppercase tracking-[0.24em] text-white transition hover:bg-[#7c3aed]">🔊 Ouvir Informações</button>
                        <button onclick="window.location.href = '{{ route('pokedex.show') }}';"
                            class="rounded-full bg-[#facc15] px-6 py-3 text-sm font-semibold uppercase tracking-[0.24em] text-[#0f172a] transition hover:bg-[#fde68a]">Random Pokemon</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        const pokemonData = {
            id: @json($pokemon['id']),
            name: @json(ucfirst($pokemon['name'])),
            description: @json($description),
            stats: @json($pokemon['stats']),
        ...
        function speakPokemonInfo() {
            if (window.voiceAPI && typeof window.voiceAPI.speakPokemonInfo === 'function') {
                window.voiceAPI.speakPokemonInfo(pokemonData);
            } else {
                console.warn('voiceAPI não disponível');
            }
        }

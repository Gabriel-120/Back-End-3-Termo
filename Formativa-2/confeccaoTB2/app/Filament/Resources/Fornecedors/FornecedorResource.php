...
class FornecedorResource extends Resource
{
    ...
    public static function table(Table $table): Table
    {
        ...
        return $table->columns([
            ...
            TextColumn::make('telefone')->label('Telefone')->formatStateUsing(fn (string $state): string => $state ? '(' . substr($state, 0, 2) . ') ' . substr($state, 2, 5) . '-' . substr($state, 7) : ''),
            TextColumn::make('celular')->label('Celular')->formatStateUsing(fn (string $state): string => $state ? '(' . substr($state, 0, 2) . ') ' . substr($state, 2, 5) . '-' . substr($state, 7) : ''),
